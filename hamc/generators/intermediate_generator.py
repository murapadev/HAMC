...
class IntermediateGenerator(BaseGenerator):
    ...
    def collapse(self) -> bool:
        ...
        while attempts < max_attempts:
            ...
            if not target:
                # Double check that all cells are collapsed
                uncollapsed = []
                for r in range(self.height):
                    for c in range(self.width):
                        if self.cells[r][c].entropy() >= 0:  # Not collapsed
                            uncollapsed.append((r, c))
                
                if uncollapsed:
                    self.logger.warning(f"Found {len(uncollapsed)} uncollapsed cells despite no minimum entropy")
                    # Force collapse on one of the uncollapsed cells
                    r, c = uncollapsed[0]
                    self.logger.info(f"Forcing collapse of cell at ({r},{c})")
                    if not self.cells[r][c].collapse():
                        continue
                    if not self.propagate(r, c):
                        continue
                else:
                    self.logger.info("All cells collapsed successfully")
                    return True
            else:
                r, c = target
                if min_entropy == float('inf'):
                    if not stack:
                        self.logger.error("No valid options remain and no backtrack points available")
                        return False
                ...
                self.logger.info(f"Collapsing cell at ({r},{c}) with entropy {min_entropy}")
                if not self.cells[r][c].collapse():
                    self.logger.error(f"Failed to collapse cell at ({r},{c})")
                    continue
                
                if not self.propagate(r, c):
                    self.logger.error(f"Propagation failed after collapsing cell at ({r},{c})")
                    continue
        
        self.logger.error(f"Failed to complete collapse after {max_attempts} attempts")
        return False
